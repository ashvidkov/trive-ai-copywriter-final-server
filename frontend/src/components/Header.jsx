...
const Header = ({ username = "alexey", role = "admin" }) => {
  ...
  return (
    <header className="w-full h-[60px] min-h-[60px] max-h-[62px] bg-white flex items-center justify-between px-8 shadow-sm border-b border-gray-100 z-20">
      ...
      <div className="flex-1 flex justify-center">
        <span className="text-3xl font-extrabold tracking-wide text-primary uppercase select-none">SEO AI ассистент</span>
      </div>
      {/* Справа */}
      <div className="flex items-center gap-6 min-w-[200px] justify-end">
        <div className="flex flex-col items-end mr-2">
          <span className="font-bold text-gray-800 leading-tight text-lg">{username}</span>
          <span className="text-base text-gray-500 leading-tight">{rolesMap[role] || role}</span>
        </div>
        {/* Кнопка административной панели (только для админов) */}
        {role === "admin" && (
          <button
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.2} stroke="currentColor" className="w-7 h-7 text-gray-500">
