...
# Комментарий: разбиваем по предложениям, чтобы длина чанка не превышала max_length
...
async def generate_keywords(text: str) -> str:
    system_prompt = (
        "Ты — SEO-оптимизатор, который составляет список ключевых слов и фраз для статьи. Твоя задача — выделить из текста 5–10 самых релевантных ключевых фраз. Они должны соответствовать реальным поисковым запросам, быть конкретными, не общими. Результат — одна строка, ключевые фразы через запятую. Никаких заголовков, кавычек, пунктов списка или пояснений."
    )
    user_prompt = (
        f"Извлеки 5–10 ключевых слов и фраз из следующего текста. Укажи их через запятую. Это должны быть реальные поисковые запросы по теме:\n\n{text}"
    )
    response = await client.chat.completions.create(
        model="gpt-4o",
        messages=[
...
async def generate_full_output(text: str) -> Dict[str, str]:
    # Комментарий: разбиваем текст на чанки, уникализируем каждый, собираем результат и генерируем SEO-поля
